/*
  ...
  - Added the required column `reciverName` to the `orders` table without a default value. This is not possible if the table is not empty.
  - Added the required column `reciverPhone` to the `orders` table without a default value. This is not possible if the table is not empty.
...
ALTER TABLE `orders` ADD COLUMN `paymentMethod` VARCHAR(191) NOT NULL,
    ...
    ADD COLUMN `reciverName` VARCHAR(255) NOT NULL,
    ADD COLUMN `reciverPhone` VARCHAR(255) NOT NULL,
